...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $TRXBulanIni = Carbon::now()->format('Y-m');
        $user       = auth()->user();
        ...
        if ($userRole === 'administrator' || $userRole === 'kepala restoran') {
            ...
            $pemasukanHariIni   = Pembelian::whereDate('tgl_transaksi', $trxHariIni)
                ->sum('total_harga');
            $pemasukanBulanIni  = Pembelian::whereMonth('tgl_transaksi', Carbon::now()->month)
                ->sum('total_harga');
            ...
            $grafikBulanan = Pembelian::selectRaw('DATE_FORMAT(tgl_transaksi, "%Y-%m") as month, COUNT(*) as total')
                ->whereYear('tgl_transaksi', Carbon::now()->year)
                ->groupBy('month')
                ->get()
                ->map(function ($data) {
                    $data->month = Carbon::parse($data->month . '-01')->format('Y-m');
                    $data->total = (int)$data->total;
                    return $data;
                });
        } else {
            ...
            $grafikBulanan = Pembelian::where('cabang_id', $user->cabang_id)->selectRaw('DATE_FORMAT(tgl_transaksi, "%Y-%m") as month, COUNT(*) as total')
                ->whereYear('tgl_transaksi', Carbon::now()->year)
                ->groupBy('month')
                ->get()
                ->map(function ($data) {
