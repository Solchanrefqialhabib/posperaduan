...
class RekapPemasukanController extends Controller
{
     ...
     * Get Data 
     */
    public function getData(Request $request, Excel $excel)
    {
        $user = auth()->user();
        ...
        if ($request->has('print_pdf')) {
            ...
            $dompdf->render();
            return $dompdf->stream('rekap_pemasukan.pdf');
        }

        if ($request->has('export_excel')) {
            return $excel->download(new PembelianExport($pembelians), 'rekap_pemasukan.xlsx');
        }
